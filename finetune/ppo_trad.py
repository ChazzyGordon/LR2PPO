...
def train_model(args, model, optimizer, critic_optim, scheduler, critic_scheduler, memories, epoch):
    ...
    for _ in range(1):
        ...
        for j, (state, next_state, old_action_prob, rewards, old_value, text_emb_batch, tgts_batch) in enumerate(memories):
            model.zero_grad()
            optimizer.zero_grad()
            ...
            value = model.critic(
                text_emb_batch, None, tgts_batch, state)
...
def main():
    ...
    for epoch in range(1, args.epochs_num):
        ...
        for i, (ground_truths, query_id, features) in enumerate(train_loader):
            text_emb, _, tgts = features, None, ground_truths
            ...
            for timestep in range(args.max_timesteps):
                ...
                with torch.no_grad():
                    ...
                    next_rewards = reward_model(
                        text_emb_batch, None, tgts_batch, next_state)
                    rewards = next_rewards

                memories.append([state.clone().detach(), next_state.clone().detach(), action_prob.clone().detach(), rewards.clone().detach(
                ), value.clone().detach(), text_emb_batch.clone().detach(), tgts_batch.clone().detach()])
