...
def main():
    ...
    parser.add_argument("--train_path2", type=str, required=False,
                        help="Path of the trainset2.")
    ...
    trainset2 = LTRDataset(args, args.train_path2, is_train=True)
    ...
    train_loader3 = get_dataloader(
        args, trainset2, num_tasks, global_rank, is_train=True)
    ...
    for epoch in range(1, args.epochs_num + 1):
        ...
        for i, d in enumerate(zip(train_loader, train_loader3)):
            for j in range(2):
                text_emb, img_emb, tgts = d[j][2], None, d[j][0]
                text_emb_batch = text_emb.to(args.device)
                tgts_batch = tgts.to(args.device)
                if args.is_master and j == 1:
                    log_dict = {}
                    pbar.update(1)
                ...
                step += 1
                if (i + 1) % args.report_steps == 0 and j==1:
