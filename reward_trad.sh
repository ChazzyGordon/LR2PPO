...
OUTPUT_MODEL_DIR=reward_ckpt_trad/$1
mkdir -p ${OUTPUT_MODEL_DIR}

LOG_DIR=reward_logs_trad/$1
mkdir -p ${LOG_DIR}
...
CUDA_VISIBLE_DEVICES=$GPUS torchrun --nproc_per_node=$NGPUS --master_port $PORT finetune/reward_trad.py \
                                   "${train_args[@]}" \
                                   "${text_args[@]}" \
                                   "${vit_args[@]}"
